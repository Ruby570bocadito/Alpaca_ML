...
class MetricsManager:
    ...
    def collect_metrics(self):
        """Recoge las métricas actuales del sistema."""
        try:
            logger.debug("Recogiendo métricas del sistema...")
            # Aquí podrías añadir lógica de recopilación real (rendimiento, equity, etc.)
            self.update_metrics()
        except Exception as e:
            logger.error(f"Error al recoger métricas: {e}", exc_info=True)

    def save_metrics(self):
        """Guarda las métricas en disco o base de datos."""
        try:
            logger.debug("Guardando métricas del sistema...")
            # Si ya tienes lógica de guardado (por ejemplo en update_metrics), puedes llamarla aquí
            pass
        except Exception as e:
            logger.error(f"Error al guardar métricas: {e}", exc_info=True)

    def calculate_backtest_metrics(self, portfolio_history):
        """
        Calcula métricas básicas del backtest: retorno total, drawdown, Sharpe ratio, etc.
        """
        self.logger.info("Calculando métricas del backtest...")

        if portfolio_history is None or len(portfolio_history) == 0:
            self.logger.warning("No hay historial de portafolio disponible para métricas.")
            return {}

        # Convertir a DataFrame si viene como lista de dicts
        df = pd.DataFrame(portfolio_history)

        if 'equity' not in df.columns:
            self.logger.warning("El historial de portafolio no contiene la columna 'equity'.")
            return {}
        ...
        metrics = {
            ...
            "Days Simulated": len(df)
        }

        self.logger.info(f"Resultados del backtest: {metrics}")
        return metrics
